...
const PostView = ({ children }) => {
  return <PostViewBlock>{children}</PostViewBlock>;
};
...
const PostViewBlock = styled.div`
  ...
  a {
    ...
    &[href] {
      box-shadow: inset 0 -4px 0 ${({ theme }) => theme.colors.primaryColor || 'rgba(0,0,0,0.0)'};
      transition: box-shadow 0.2s, color 0.2s;
    }

    &:hover {
      text-decoration: none;
      box-shadow: inset 0 -20px 0 ${({ theme }) => theme.colors.primaryColor || 'rgba(0,0,0,0.0)'};
      color: ${({ theme }) => theme.colors.accentColor || '#000'};
    }
  ...
  table {
    ...
    td {
      border: 1px solid
        ${({ theme }) => lighten(0.8, theme.colors.accentColor || '#000')};
    }
  }
`;
